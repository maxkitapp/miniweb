...
public class DataUtils {
	public static synchronized List<Body> cwbStoreToBody(CwbDataStore cwbStore, String pagename) {
		...
		for(int i = 0; i < 10; i++) {
			...
			String desc = time.getElementValue(); // "晴天。 降雨機率 0%。 溫度攝氏19度。 稍有寒意。 偏北風 平均風速3至4級(每秒6公尺)。 相對濕度為68%。"
	...
	public static synchronized List<Body> contactsToBody(String keyword, List<Contact> contacts) {
		...
		for(Contact contact : contacts) {
			...
			sb.append(generateDisplayname(contact.getFirstName(), contact.getLastName())).append(" (").append(contact.getAccname()).append(")");
